...
Usuario.autenticar = function(data, callback) {
  if(database) {
    var sql = "SELECT * FROM Usuario WHERE nick = ? AND password = ?";
    database.query(sql, [data.nick, data.password],
    function(error, resultado) {
      if(error) {
        throw error;
      } else {
...
Usuario.update = function(data, callback) {
  if(database) {
    ...
    +"nick = ?, password = ? WHERE idUsuario = ?";
    database.query(sql,
    [data.nick, data.password, data.idUsuario],
    function(error, resultado) {
      if(error) {
        throw error;
      } else {
